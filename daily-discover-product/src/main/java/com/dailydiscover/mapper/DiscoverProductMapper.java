...
public interface DiscoverProductMapper {
    ...
    @Select("SELECT p.* FROM products p " +
            ...
            "WHERE drp.recommend_date = #{date} AND p.is_active = true " +
    ...
    @Select("SELECT p.* FROM products p " +
            "INNER JOIN daily_recommend_products drp ON p.id = drp.product_id " +
            "WHERE drp.theme_id = #{themeId} AND p.is_active = true " +
            "ORDER BY drp.sort_order ASC")
    ...
    // 插入商品
    @Insert("INSERT INTO products (name, description, price, image_url, category, is_active, view_count, like_count, favorite_count, created_at, updated_at) " +
            "VALUES (#{name}, #{description}, #{price}, #{imageUrl}, #{category}, #{isActive}, #{viewCount}, #{likeCount}, #{favoriteCount}, #{createdAt}, #{updatedAt})")
    ...
    @Update("UPDATE products SET name = #{name}, description = #{description}, price = #{price}, image_url = #{imageUrl}, " +
            "category = #{category}, is_active = #{isActive}, view_count = #{viewCount}, like_count = #{likeCount}, " +
    ...
    // 搜索商品
    @Select("SELECT * FROM products WHERE (name LIKE CONCAT('%', #{keyword}, '%') OR description LIKE CONCAT('%', #{keyword}, '%')) AND is_active = true")
    ...
    // 根据时间段查找商品
    @Select("SELECT * FROM products WHERE time_slot = #{timeSlot} AND is_active = true ORDER BY created_at DESC")
    List<ProductEntity> findByTimeSlot(@Param("timeSlot") String timeSlot);
    
    // 批量插入商品
    @Insert("<script>" +
            "INSERT INTO products (name, description, price, image_url, category, is_active, view_count, like_count, favorite_count, created_at, updated_at) VALUES " +
            "<foreach collection='products' item='product' separator=','>" +
            "(#{product.name}, #{product.description}, #{product.price}, #{product.imageUrl}, #{product.category}, #{product.isActive}, #{product.viewCount}, #{product.likeCount}, #{product.favoriteCount}, #{product.createdAt}, #{product.updatedAt})" +
            "</foreach>" +
            "</script>")
