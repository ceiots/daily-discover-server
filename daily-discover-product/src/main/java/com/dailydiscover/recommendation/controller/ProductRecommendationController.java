...
public class ProductRecommendationController {
    ...
    public ResponseEntity<GuidedRecommendationResponseDTO> getGuidedRecommendation(
            @RequestParam String sessionId,
            @RequestParam String intentLabel,
            @RequestParam(required = false) Integer round,
            @RequestParam(required = false) Integer limit,
            @RequestParam(required = false) Long userId) {
        try {
            Integer effectiveRound = round != null ? round : 1;
            Integer effectiveLimit = limit != null ? limit : 6;
            
            // 获取商品
            List<GuidedProductDTO> products = productRecommendationService.getGuidedProducts(
                sessionId, intentLabel, effectiveLimit, userId
            );
            
            // 基于商品生成下一级推荐词
            List<GuidedOptionDTO> nextOptions = productRecommendationService.generateNextOptions(
                products, intentLabel, effectiveRound
            );
            
            GuidedRecommendationResponseDTO response = new GuidedRecommendationResponseDTO();
            response.setProducts(products);
            response.setNextOptions(nextOptions);
            response.setCurrentPath(Collections.singletonList(intentLabel));
            response.setRound(effectiveRound);
